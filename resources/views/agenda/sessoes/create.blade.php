@extends('layouts.app')

@section('title', 'Nova Sessão')
@section('page-title', 'Nova Sessão')

@section('header-actions')
    <a href="{{ route('agenda.index') }}" class="btn btn-outline-secondary btn-sm">&larr; Voltar à Agenda</a>
@endsection

@section('content')
<div class="row justify-content-center">
    <div class="col-lg-8">
        <div class="card border-0 shadow-sm">
            <div class="card-body p-4">
                <form action="{{ route('sessoes.store') }}" method="POST" id="form-sessao">
                    @csrf

                    @if ($errors->any())
                    <div class="alert alert-danger">
                        <ul class="mb-0 ps-3">
                            @foreach ($errors->all() as $e)
                                <li>{{ $e }}</li>
                            @endforeach
                        </ul>
                    </div>
                    @endif

                    <div class="row g-3">

                        {{-- Paciente --}}
                        <div class="col-12">
                            <label class="form-label fw-semibold">Paciente <span class="text-danger">*</span></label>
                            <select name="paciente_id" class="form-select @error('paciente_id') is-invalid @enderror" required>
                                <option value="">Selecione...</option>
                                @foreach ($pacientes as $pac)
                                    <option value="{{ $pac->id }}" @selected(old('paciente_id') == $pac->id)>{{ $pac->nome }}</option>
                                @endforeach
                            </select>
                            @error('paciente_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        {{-- Profissional --}}
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Profissional <span class="text-danger">*</span></label>
                            <select name="profissional_id" id="sel-profissional"
                                    class="form-select @error('profissional_id') is-invalid @enderror" required>
                                <option value="">Selecione...</option>
                                @foreach ($profissionais as $prof)
                                    <option value="{{ $prof->id }}"
                                            data-especialidade="{{ $prof->especialidade->value }}"
                                            data-duracao="{{ $prof->duracao_sessao_min }}"
                                            @selected(old('profissional_id', $profissionalSelect) == $prof->id)>
                                        {{ $prof->user->name }} ({{ $prof->especialidade->label() }})
                                    </option>
                                @endforeach
                            </select>
                            @error('profissional_id') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        {{-- Especialidade --}}
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Especialidade <span class="text-danger">*</span></label>
                            <select name="especialidade" id="sel-especialidade"
                                    class="form-select @error('especialidade') is-invalid @enderror" required>
                                <option value="">Selecione...</option>
                                @foreach ($especialidades as $esp)
                                    <option value="{{ $esp->value }}" @selected(old('especialidade') == $esp->value)>
                                        {{ $esp->label() }}
                                    </option>
                                @endforeach
                            </select>
                            @error('especialidade') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        {{-- Data e Hora --}}
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Data e Hora <span class="text-danger">*</span></label>
                            <input type="datetime-local" name="data_hora"
                                   value="{{ old('data_hora', $dataSelecionada && $horaSelecionada ? $dataSelecionada.'T'.$horaSelecionada : '') }}"
                                   class="form-control @error('data_hora') is-invalid @enderror" required>
                            @error('data_hora') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        {{-- Duração --}}
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Duração (minutos) <span class="text-danger">*</span></label>
                            <input type="number" name="duracao_min" id="inp-duracao"
                                   value="{{ old('duracao_min', 50) }}"
                                   min="15" max="240" step="5"
                                   class="form-control @error('duracao_min') is-invalid @enderror" required>
                            @error('duracao_min') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        {{-- Tipo --}}
                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Tipo <span class="text-danger">*</span></label>
                            <select name="tipo" class="form-select @error('tipo') is-invalid @enderror" required>
                                <option value="avulsa"   @selected(old('tipo','avulsa') === 'avulsa')>Avulsa</option>
                                <option value="plano"    @selected(old('tipo') === 'plano')>Plano</option>
                                <option value="reposicao" @selected(old('tipo') === 'reposicao')>Reposição</option>
                            </select>
                            @error('tipo') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        {{-- Observações --}}
                        <div class="col-12">
                            <label class="form-label fw-semibold">Observações</label>
                            <textarea name="observacoes" rows="2"
                                      class="form-control @error('observacoes') is-invalid @enderror"
                                      maxlength="1000">{{ old('observacoes') }}</textarea>
                            @error('observacoes') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="col-12 d-flex gap-2 justify-content-end">
                            <a href="{{ route('agenda.index') }}" class="btn btn-outline-secondary">Cancelar</a>
                            <button type="submit" class="btn btn-primary">Agendar Sessão</button>
                        </div>
                    </div>
                </form>
            </div>
        </div>
    </div>
</div>
@endsection

@push('scripts')
<script>
document.getElementById('sel-profissional').addEventListener('change', function () {
    const opt = this.options[this.selectedIndex];
    if (!opt.value) return;
    // Preenche a especialidade com a do profissional
    const espSel = document.getElementById('sel-especialidade');
    for (let o of espSel.options) {
        if (o.value === opt.dataset.especialidade) { o.selected = true; break; }
    }
    // Preenche duração padrão
    const dur = opt.dataset.duracao;
    if (dur) document.getElementById('inp-duracao').value = dur;
});
</script>
@endpush
